...
## Phase 3C implementation observation — 2026-09-15

The generated `credentialRevocationWitnessV2` callback returns exactly `[PS, CredentialRevocationWitnessV2]` in the ordered schema above. The adapter copies every byte array and freezes structural containers in one detached snapshot; generated execution calls it once and preserves private input. There is no implemented qualification witness yet.

The sole new explicit disclosure is `disclose(newRoot)` for the revocationRoot write. Actual runtime partitioning exposes context, issuer count/root, credential count/root, current revocation root and the final root write. The exact 21-operation query has six ledger reads, no kernel address read or nullifier Set access, and no log/time/asset/cross-contract effect. Ephemeral private atoms (including both indices and all three paths) are absent from inspected public locations, with positive private-witness and public-final-root controls. Source inspection supplements, rather than replaces, those structural checks. See the [Phase 3C report](../../development/phase-3c-revoke-credential-report.md).

## Phase 3D implementation observation — 2026-09-15

`qualificationWitnessV2` now returns exactly `[PS, QualificationWitnessV2]` with the ordered five fields above: credential, subjectSecret, issuerMembership, credentialMembership, revocationPath. One detached snapshot copies byte arrays and freezes structural containers; no authority/issuer secret is requested. Generated argument/result schemas retain the exact ten-field request and two digests.

The only new disclosures are the public requestIssuedAt/requestExpiresAt passed to blockTimeGte/blockTimeLt. Actual runtime partition inspection finds seven address/configuration/root/counter reads and two time comparisons, no writes, Set access, logs or asset/call effects. Private non-public atoms are absent from inspected argument/result/query/partition/effect/state locations. Fixed protocol/qualification constants overlap public request metadata and are not unique private canaries. Three private time variants leave the time-query program unchanged. Full request values occur in the generated argument descriptor; this does not claim all ten are separately exposed as raw chain-query fields. See the [Phase 3D report](../../development/phase-3d-prove-qualification-report.md). No proof or network verification is established.
