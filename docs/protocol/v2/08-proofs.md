...
## Phase 3D implementation status — 2026-09-15

The production circuit now implements the ordered request checks, current issuer/credential/same-index NOT_REVOKED membership, complete private package validation and persistent subject-commitment opening, all eight time constraints, and both typed digests. It writes none of the nine ledger fields. Generated simulation covers inclusive issuance, exclusive deadline/expiry, equal credential/request deadline, zero expiry, private interval failures and Uint<64> limits. It uses no issuer or authority secret.

The ten-field request is intentionally non-secret caller metadata. Its generated argument descriptor contains all ten fields; the query program separately contains the contract/context/current state reads and two public request-time comparisons. Only those request timestamps create time bounds. Complete proof/transcript verification and verifier-issued trusted time/challenge single use remain future capability-gated work. Source compilation and simulation do not close R61 or establish an actual proof. Exact evidence is in the [Phase 3D report](../../development/phase-3d-prove-qualification-report.md).
