...
describe("registerIssuerV2 compiled contract logic",()=>{
  ...
  it("incremental ABI exports only the two approved registration circuits and retains issuer witness fields",()=>{
    const f=fixture();expect(Object.keys(f.contract.circuits)).toEqual(["registerIssuerV2","registerCredentialV2"]);
    expect(Object.keys(f.contract.provableCircuits)).toEqual(["registerIssuerV2","registerCredentialV2"]);
    const source=readFileSync("contracts/just-proof.compact","utf8");
    expect(source.match(/export circuit/g)).toHaveLength(2);
