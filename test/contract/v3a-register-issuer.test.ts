...
describe("registerIssuerV2 compiled contract logic",()=>{
  ...
  it("ABI exports only registerIssuerV2 and exactly the approved witness fields",()=>{
    const f=fixture();expect(Object.keys(f.contract.circuits)).toEqual(["registerIssuerV2"]);
    expect(Object.keys(f.contract.provableCircuits)).toEqual(["registerIssuerV2"]);
    const source=readFileSync("contracts/just-proof.compact","utf8");
    expect(source.match(/export circuit/g)).toHaveLength(1);
    expect(source.match(/disclose\(/g)).toHaveLength(4);
